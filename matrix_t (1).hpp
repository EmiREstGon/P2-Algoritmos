...
template <class T>
void matrix_t<T>::resize(const int m, const int n)  // llamada de la función "resize" que pertenece a la clase "matrix_t<T>"
{
...
template <class T>
T& matrix_t<T>::at(const int i, const int j)  // llamada del setter "at" que pertenece a la clase "matrix_t<T>"
{
...
template <class T>
T& matrix_t<T>::operator()(const int i, const int j)  // llamada del getter "operator" que pertenece a la clase "matrix_t<T>"
{
  return at(i, j);
}
...
template <class T>
void matrix_t<T>::write(ostream& os) const  // salida de la matriz "matrix_t<T>"
{
...
template <class T>
void matrix_t<T>::read(istream& is)  // lectura de la matriz "matrix_t<T>"
{
  is >> m_ >> n_;
  resize(m_, n_);
  for (int i = 1; i <= get_m(); ++i)
...
void matrix_t<T>::multiply(const matrix_t<T>& A, const matrix_t<T>& B)  // producto matricial
{
  ...
  for (size_t i = 0; i < result.get_m(); ++i) {
    for (size_t j = 0; j < result.get_n(); ++j) {
      for (size_t k = 0; k < A.get_n(); ++k) {
        result(i, j) = result(i, j) + A(i, k) * B(j, k); // producto matricial (A * B)
      }
    }
  }
  return result;
